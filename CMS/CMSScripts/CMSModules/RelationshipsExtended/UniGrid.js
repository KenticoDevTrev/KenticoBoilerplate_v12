...
cmsdefine(['CMS/WebFormCaller', 'jQuery'], function (webFormCaller, $) {
    ...
    var Module = function (data) {
        var config = data,
            ...
            // Custom Property added by Trevor Fayas, see InitSorting.Update for why
            $customMoveArgs = $('#'+ config.hdnCustomMoveArgs),
        ...
        initSorting = function () {
            ...
            gridBody.sortable({
                ...
                update: function (event, ui) {
                    ...
                    // Because there's special hardcoded logic to allow a differnet value for the command,
                    // of which i can't seem to override, it will only trigger teh command if i send it the
                    // basic command argument (the ID), so i'll set a different hidden value to the 
                    // actual value, and look that up on the OnAction step if it's a "custom_move" action

                    //command('custom_move', $commandArg.val() + ':' + previousIndex + ':' + newIndex);
                    $customMoveArgs.val($commandArg.val() + ':' + previousIndex + ':' + newIndex);
                    command('custom_move', $commandArg.val());
                }
            }).disableSelection();
